...
class Student {
    constructor(name) {
     ...
     addMark(mark, subject) {
        ...
        return this.marks[subject] !== undefined ? this.marks[subject].reduce((acc, item, arr) => acc + item / arr.length, 0) : 0;
    }

    getAverage() {
        return Object.keys(this.marks).reduce((acc, item, arr) => acc + this.getAverageBySubject([item]) / arr.length, 0);
    }
}